...
for i in range(1, 5):
    f_name = 'InputOutputExamples\\InputOutputExamples\\TestCase{0}\\TrainingExamples1.csv'.format(str(i))
    ...
    tree.format = 'png'
    tree.render('InputOutputExamples\\InputOutputExamples\\TestCase{0}\\mytree.png'.format(str(i)))

for i in range(5, 11):
    f_name = 'InputOutputExamples\\InputOutputExamples\\TestCase{0}\\TrainingExamples.csv'.format(str(i))
    print("Reading file {}".format(f_name))
    df = pd.read_csv(f_name, delim_whitespace=True)
    labels = df['PlayTennis'].tolist()
    df.drop('PlayTennis', axis=1, inplace=True)
